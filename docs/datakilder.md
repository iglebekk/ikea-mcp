...
- **Fallback**: produktsiden `https://www.ikea.com/{cc}/{lc}/p/-{artikkelnr}/` har JSON-LD (`application/ld+json`) med navn, beskrivelse, pris og bilder — kan implementeres i `IkeaApi::productDetails()` hvis PIP-endepunktet forsvinner
...
- **Header**: `X-Client-Id` — en **offentlig** klientidentifikator som IKEA.com sin frontend bruker (ikke en hemmelighet). Konfigurerbar via `IKEA_AVAILABILITY_CLIENT_ID`
